...
def main():
    ...
    lap_rect =pygame.Rect(640,0,160,height)
    delay = 100
    laps = [0]
    x1=0
    y1=0
    ...
    isrunning = False
    # WEBカメラからリアルタイム検出
    results = model(1 , show=False, stream = True,classes = [target])#11 stopsign
    ...
    for result in results:
        ...
        screen.fill((255,255,255), lap_rect)
        pygame.draw.line(screen, (0,95,0), (640/2,0), (640/2,height), 5)
        for lap in laps:
            if laps.index(lap)==0:
                continue
            lap_txt = font.render(f"{lap/10}s /{laps.index(lap)}", True, (0,0,0))
            screen.blit(lap_txt,(640,laps.index(lap)*30))
